...
FFramePackage ULagCompensationComponent::InterpBetweenFrames(const FFramePackage& OlderFrame, const FFramePackage& YoungerFrame, float HitTime)
{
	...
	FFramePackage InterpFramePackage;//对应于HitTime的FFramePackage，通过插值获得
...
{
	if (HitCharacter == nullptr || Package.Character == nullptr) return FServerSideRewindResult();
...
FShotgunServerSideRewindResult ULagCompensationComponent::ShotgunConfirmHit(const TArray<FFramePackage>& FramePackages, const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations)
{
	...
	TArray<FFramePackage> CurrentFrames;//存储每个角色当前的frame
	for (auto& Frame : FramePackages)//每个Frame对应HitCharacters中的一个Character
	{
		FFramePackage CurrentFrame;
	...
	for (auto& Frame : FramePackages)
	{
		...
		UBoxComponent* HeadBox = Frame.Character->HitCollisionBoxes[FName("head")];
		HeadBox->SetCollisionEnabled(ECollisionEnabled::NoCollision);//防止下面重复计算爆头次数
	}
	...
	{
		ResetHitBoxes(Frame.Character, Frame);//这个函数中会禁用HitBoxes的碰撞
...
void ULagCompensationComponent::SaveFramePackage()
{
	...
	if (FrameHistory.Num() <= 1)
	{
		...
		SaveFramePackage(ThisFrame);//SaveFramePackage()相同的函数名 不同的输入参数
		FrameHistory.AddHead(ThisFrame);
	}
	else
	{
		float HistoryLength = FrameHistory.GetHead()->GetValue().Time - FrameHistory.GetTail()->GetValue().Time;
		while (HistoryLength > MaxRecordTime)//每一帧的时长不一样
		{
